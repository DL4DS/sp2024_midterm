...
from dataset import FTDataset   ## This is a local import from dataset.pyA
...
try:
    if os.path.exists(f"{FT_SAVE_PATH}/best_model"):
        model = BlipForConditionalGeneration.from_pretrained(f"{FT_SAVE_PATH}/best_model", cache_dir=CACHE_DIR)
    else:
...
logger = Logger(f"{FT_SAVE_PATH}/logs.log")
...
def get_val_examples(vizwizEval, vizwizRes, plot_captions_dict, epoch, method="CIDEr"):
    ...
    save_image_captions(
        best_img_and_captions, f"{FT_SAVE_PATH}/examples/epoch_{epoch}/best/"
    )
    save_image_captions(
        worst_img_and_captions, f"{FT_SAVE_PATH}/examples/epoch_{epoch}/worst/"
    )
    save_image_captions(
        first_3_img_and_captions, f"{FT_SAVE_PATH}/examples/epoch_{epoch}/first_3/"
    )
...
for epoch in range(30):
    ...
    if epoch % 3 == 0:
        ...
        if score > best_score:
            best_score = score
            if torch.cuda.device_count() > 1:
                model.module.save_pretrained(f"{FT_SAVE_PATH}/best_model")
            else:
                model.save_pretrained(f"{FT_SAVE_PATH}/best_model")
