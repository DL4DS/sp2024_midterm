...
from dataset import PEFTDataset
...
create_directory(PEFT_SAVE_PATH)  # src/base/helpers.py
create_directory(PEFT_SAVE_PATH + "/examples")

# The path below points to the location where the model was saved
MODEL_PATH = f"{PEFT_SAVE_PATH}/best_model"
PEFT_CONFIG_PATH = f"{PEFT_SAVE_PATH}/best_model"

# Load the saved lora checkpoint
m = BlipForConditionalGeneration.from_pretrained(MODEL_PATH, cache_dir=CACHE_DIR)
model = PeftModel.from_pretrained(model=m, model_id=PEFT_CONFIG_PATH, cache_dir=CACHE_DIR)
...
with open(PEFT_SAVE_PATH + "/test_captions_revised_continued2.json", "w") as f:
